...
- **情绪分析（v0.0.2）**：对话情绪分析（valence-arousal）、记忆深化分层与定时巩固。
...
- **深度研究与定时任务（v0.0.4）**：把「研究一个主题」拆成规划→多源检索→逐源提炼→反思补搜→大纲→分节写作的多阶段流水线,自动产出结构化带引用的报告;定时任务可自动运行并推送到已启用渠道（含飞书）,手动研究由用户决定是否推送;报告可自动归档到 GitCode。
- **真人对话模式（v0.0.4）**：全局开关,让对话切到真人微信聊天风格(口语 / 多气泡逐条连发 / 不甩 markdown 报告腔)。
